import 'package:classy_parking/presentation/screens/parking_payment/prev/prev_pay_view_model.dart';
...
class PrevPayScreen extends StatelessWidget {
  const PrevPayScreen({super.key});

  @override
  Widget build(BuildContext context) {
    final Map<String, dynamic>? extra = GoRouterState.of(context).extra as Map<String, dynamic>?;

    // 전달받은 데이터를 SelectedProductInfo 객체로 변환
    final SelectedProductInfo productInfo = SelectedProductInfo(
      parkingName: extra?['parkingName'] as String? ?? "주차장 이름 없음",
      productTitle: extra?['productTitle'] as String? ?? "상품 제목 없음",
      availableTime: extra?['availableTime'] as String? ?? "시간 정보 없음",
      selectedFee: extra?['selectedFee'] as int? ?? 0,
    );

    return ChangeNotifierProvider(
      // 뷰 모델 생성자에 추출한 데이터를 전달
      create: (_) => PrevPayViewModel(productInfo: productInfo),
      child: const _PrevPayView(),
    );
  }
}

class _PrevPayView extends StatelessWidget {
  const _PrevPayView();

  @override
  Widget build(BuildContext context) {
    final viewModel = Provider.of<PrevPayViewModel>(context);
    final info = viewModel.paymentInfo;

    return Scaffold(
      appBar: CustomSubAppBar(title: '결제하기'),
      bottomNavigationBar: Consumer<PrevPayViewModel>(
